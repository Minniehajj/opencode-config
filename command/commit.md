...
1. **Check command mode**:
   - If user provides $ARGUMENTS (a simple message), skip to step 2.
2. **Run pre-commit validation**:
   - Execute npm run build in any changed services that contain a package.json file and are not ignored by .gitignore
...
5. **Generate commit message**:
   - Choose an appropriate type from the reference below
