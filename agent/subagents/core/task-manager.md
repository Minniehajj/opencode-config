...
- Tasks without status check -> Duplicate work, conflicts
  ...
  <interaction_protocol>
    <with_meta_agent>
      ...
      - Do NOT read `context/core/workflows/delegation.md` or follow delegation workflows.
    ...
    <stage id="0" name="ContextLoading">
      ...
      <process>
        ...
        2. Check current task state:
           ```bash
           npx ts-node --compiler-options '{"module":"commonjs"}' skill/task-management/scripts/task-cli.ts status
           ```
    ...
    <stage id="2" name="JSONCreation">
      ...
      <process>
        1. Create directory:
         ...
         3. Create subtask_NN.json for each task:
             ...
             **AGENT FIELD SEMANTICS**:
              ...
              **FRONTEND RULE**: If a task involves UI design, styling, or frontend implementation:
              ...
              2. Include `context/ui/web/ui-styling-standards.md` and `context/core/workflows/design-iteration.md` in `context_files`.
         ...
         4. Validate with CLI:
           ```bash
           npx ts-node --compiler-options '{"module":"commonjs"}' skill/task-management/scripts/task-cli.ts validate {feature}
           ```
    ...
    <stage id="3" name="Verification">
      ...
      <process>
        ...
        3. If all criteria pass:
           ```bash
           npx ts-node --compiler-options '{"module":"commonjs"}' skill/task-management/scripts/task-cli.ts complete {feature} {seq} "{summary}"
           ```
        ...
        5. Check for next task:
           ```bash
           npx ts-node --compiler-options '{"module":"commonjs"}' skill/task-management/scripts/task-cli.ts next {feature}
           ```
      </process>
    ...
    <stage id="4" name="Archiving">
      ...
      <process>
        1. Verify all tasks complete:
           ```bash
           npx ts-node --compiler-options '{"module":"commonjs"}' skill/task-management/scripts/task-cli.ts status {feature}
           ```
...
Script location: `skill/task-management/scripts/task-cli.ts`
</cli_integration>
