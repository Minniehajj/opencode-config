...
task(
  ...
  prompt="Fetch Drizzle ORM docs: PostgreSQL setup with modular schemas
  
  Focus on: Installation | DB connection | Schema patterns | Migrations | TypeScript config
  Context: Next.js commerce site with PostgreSQL"
)
```
...
task(
  ...
  prompt="Fetch Next.js docs: App Router with Server Actions
  
  Focus on: Installation | Directory structure | Server Actions | Data fetching | Route handlers
  Context: Commerce site with order processing"
)
```
...
task(
  ...
  prompt="Fetch Better Auth docs: Next.js App Router integration with Drizzle
  
  Focus on: Installation | App Router setup | Drizzle adapter | Session mgmt | Route protection
  Context: Adding auth to Next.js commerce with Drizzle ORM"
)
```
...
Don't:
- Do not assume API based on training data
- Do not use deprecated patterns
- Do not skip version-specific requirements
...
Verify: Package installs | Imports work | API calls match docs | Error handling | Integration with other packages
...
```
User Request: "Build Next.js commerce with Drizzle"
                    |
                    v
  STEP 1: ContextScout
  -> Search internal context (context/)
  -> Find project standards, patterns, workflows
   ...
   context                       |
                       ...
                       YES               NO
                        |                 |
                        v                 v
              STEP 2: ExternalScout   Report: No context
              (MANDATORY)             available
                        |
                        v
              Fetch: Next.js docs
...
2. ExternalScout: "Fetch TanStack Query docs: Installation, Next.js App Router setup with Server Components"
...
Task: "Add authentication with Better Auth to Next.js commerce site"
...
| Error Type | Process |
|------------|---------|
| Package Installation | ExternalScout: installation docs -> Verify package name/version -> Check peer deps -> Propose fix with current steps |
| Import/Module | ExternalScout: import patterns -> Check current API exports -> Verify import paths -> Propose fix with current imports |
| API/Configuration | ExternalScout: API docs -> Check current signatures -> Verify config format -> Propose fix with current patterns |
...
- Cache useful docs in `context/development/frameworks/` if frequently used
...
Rule: If it's not in `context/`, use ExternalScout.

---

### "What if I already know the library?"

Do not rely on training data - it's outdated.
...
| Location | `context/` | Internet (Context7, official docs) |
...
See: `skill/context7/library-registry.md`
...
- ExternalScout Agent: `agent/subagents/core/externalscout.md`
- Library Registry: `skill/context7/library-registry.md`
- ContextScout Agent: `agent/subagents/core/contextscout.md`
- Code Standards: `context/core/standards/code.md`
